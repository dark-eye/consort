...
import "../components"
import "../components/jslibs/graphqlAPI.js" as GraphQLAPI
...
Kirigami.Page {
	...
	property var instanceType : "diaspora"

	Component.onCompleted: getSample ()
	
	WorkerScript {
		id:asyncProcess
		source:'../components/jslibs/ASyncProcessing.js'
		onMessage:instanceList.addToList (  messageObject.reply );
	}
	...
	CachedHttpRequest {
		...
		getData: GraphQLAPI.constructQuery({"variables":{"name":InstancePicker.instanceType}})
		...
		function addToList ( list ) {
			...
			for ( var i = 0; i < list.length; i++ ) {
				newModel.push(
				 {
					"text": list[i].domain,
					"subtitle": list[i].country != null ? list[i].country : "",
